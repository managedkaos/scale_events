...
class ScaleScheduleSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for event in response.xpath('//div[contains(@class, "calendar-")]'):
            speaker = event.xpath('.//div[contains(@class, "views-field-nothing")]//a/text()').get()
            title = event.xpath('.//div[contains(@class, "views-field-title")]//a/text()').get()
            url = event.xpath('.//div[contains(@class, "views-field-title")]//a/@href').get()
            ...
            if title and speaker and url:
                ...
                yield response.follow(url, self.parse_event_details, meta={'item': event_item})
    ...
    def parse_event_details(self, response):
        ...
        item["day"] = response.xpath('//div[contains(@class, "views-field-field-time")]//span[@class="date-display-single"]/text()').get()
        item["time"] = response.xpath('//div[contains(@class, "views-field-field-time")]//span[@class="date-display-start"]/text()').get()
        item["room"] = response.xpath('//div[contains(@class, "views-field-field-room")]//div[@class="field-content"]/text()').get()
        item["topic"] = response.xpath('//div[contains(@class, "field-content terms")]/a/text()').get()
